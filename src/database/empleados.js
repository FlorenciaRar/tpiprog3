...
export default class Empleados {
  ...
  buscarId = async (idUsuario) => {
    const sql =
      "SELECT idUsuario, nombre, apellido, correoElectronico, idTipoUsuario FROM usuarios WHERE idUsuario = ? AND idTipoUsuario = 2 AND activo = 1;";
  ...
  buscarEnOficina = async ({ idUsuario, idOficina }) => {
    const sql =
      "SELECT u.idUsuario, u.nombre, u.apellido, u.correoElectronico, u.idTipoUsuario FROM usuarios AS u JOIN usuarios_oficinas AS uo ON u.idUsuario = uo.idUsuario JOIN oficinas AS o ON uo.idOficina = o.idOficina WHERE u.idUsuario = ? AND uo.idOficina = ? AND uo.activo = 1;";
  ...
  crear = async ({ nombre, apellido, correoElectronico, contrasenia, imagen }) => {
    const sql =
      "INSERT INTO usuarios (nombre, apellido, correoElectronico, contrasenia, idTipoUsuario, imagen, activo) VALUES  (?, ?, ?, SHA2(?, 256), 2, ?, 1);";
